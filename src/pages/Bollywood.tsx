...
const Bollywood = () => {
  ...
  const { movies: bollywoodMovies, loading, error } = useMovies("bollywood");
        ...
        <div className="max-w-7xl mx-auto">
          <div className="backdrop-blur-xl bg-card/80 border border-luxury-border rounded-2xl p-6 md:p-8 shadow-2xl">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-2">Bollywood Movies</h1>
                <p className="text-muted-foreground">Explore the best of Bollywood cinema</p>
              </div>
            </div>
          ...
          <div className="backdrop-blur-xl bg-card/60 border border-luxury-border rounded-2xl p-4 md:p-8 shadow-2xl">
            {loading ? (
              <div className="text-center py-8 md:py-12">
                <p className="text-muted-foreground text-base md:text-lg">Loading Bollywood movies...</p>
              </div>
            ) : error ? (
              <div className="text-center py-8 md:py-12">
            ...
            {!loading && !error && filteredMovies.length === 0 && searchQuery && (
              <div className="text-center py-8 md:py-12">
                <p className="text-muted-foreground text-base md:text-lg">No Bollywood movies found for "{searchQuery}"</p>
                <p className="text-muted-foreground/60 mt-2 text-sm md:text-base">Try searching for something else</p>
              </div>
            )}
            
            {!loading && !error && bollywoodMovies.length === 0 && !searchQuery && (
              <div className="text-center py-8 md:py-12">
                <p className="text-muted-foreground text-base md:text-lg">No Bollywood movies available</p>
      ...
      <section className="relative z-10 px-4 md:px-6 pb-12">
        <div className="max-w-7xl mx-auto text-center">
          <div className="backdrop-blur-xl bg-card/40 border border-luxury-border rounded-2xl p-6 shadow-xl">
            ...
            <p className="text-muted-foreground mb-6">Join our Telegram group for instant notifications on new Bollywood movies</p>
            <TelegramJoin 
              variant="default" 
